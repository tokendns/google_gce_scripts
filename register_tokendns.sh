...
# Register A record
curl --get "https://api.tokendns.co/v1/reserve" -d apikey=${apikey} -d name=${name} -d domain=${domain} -d type=A -d content=${ipv4}

# Register AAAA record (if IPv6 is available)
if [ -n "${ipv6}" ]; then
	curl --get "https://api.tokendns.co/v1/reserve" -d apikey=${apikey} -d name=${name} -d domain=${domain} -d type=AAAA -d content=${ipv6}
fi
